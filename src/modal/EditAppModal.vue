<template>
  <modal-layout :show="visibly" @after-enter="onActive" @after-leave="onLeave">
    <modal-form-layout
      :width="400"
      :title="$t('modal.EditAppModal.rkc0hw')"
      confirm-:text="$t('modal.EditAppModal.NWonXP')"
...
<script setup lang="ts">
  ...
  import { AppInfo } from '@/api/type';

  const { item } = defineProps<{ item: AppInfo; visibly: boolean }>();
  ...
  const formState = ref<AppInfo>({ name: '' } as AppInfo);
  const formRef = ref<FormInst | null>(null);

  const onActive = () => {
    formState.value = { ...toRaw(item) };
  };
  const onLeave = () => {
    formState.value = { name: '' } as AppInfo;
  };
  const submit = ({ done }) => {
    formRef.value?.validate((errors) => {
      if (!errors) {
        editDevAppApi(formState.value)
          .then((res) => {
